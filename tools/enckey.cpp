...
std::pair<uint32_t, uint32_t> encode_key(uint64_t key, unsigned i)
{
    ...
    return {
        ((high_oi & DISKID_HIGH_ROTATION_MASK) << DISKID_HIGH_ROTATION)
        ...
        | (((high_part >> SECRET_HIGH_PART_SHIFT) & SECRET_HIGH_PART_MASK) << SECRET_HIGH_PART)
    };
}
