...
uint64_t
decode_key(uint32_t disk_id, uint32_t secret)
{
    uint32_t low_part = (disk_id >> DISKID_LOW_PART) & DISKID_LOW_PART_MASK;
    uint32_t high_part =
        (((secret >> SECRET_HIGH_PART) & SECRET_HIGH_PART_MASK)
         << SECRET_HIGH_PART_SHIFT) |
        ((disk_id >> DISKID_HIGH_PART) & DISKID_HIGH_PART_MASK);

    unsigned low_oi =
        (secret >> SECRET_LOW_ROTATION) & SECRET_LOW_ROTATION_MASK;
    unsigned high_oi =
        (disk_id >> DISKID_HIGH_ROTATION) & DISKID_HIGH_ROTATION_MASK;
