...
exe_pe_get_resource(void *rsrc, WORD type, WORD id)
{
    ...
    for (i = 0; (dir->NumberOfIdEntries > i) && (type != ent->Id); ++i, ++ent)
        ;
    ...
    ent = (exe_pe_resource_directory_entry *)(dir + 1);
    ent += dir->NumberOfNamedEntries;
    for (i = 0; (dir->NumberOfIdEntries > i) && (id != ent->Id); ++i, ++ent)
        ;
    ...
    data_ent = (exe_pe_resource_data_entry *)((char *)rsrc + ent->OffsetToData);
    data = (char *)(intptr_t)data_ent->OffsetToData;
...
{
    const char     *resource;
    const uint16_t *wstr, *end;

    char *buffptr;
    int   i;

    resource = exe_pe_get_resource(rsrc, EXE_PE_RT_STRING, (id >> 4) + 1);
    if (NULL == resource)
    {
        return -1;
    }
