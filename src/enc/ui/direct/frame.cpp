...
static ui::button  _cancel{*static_cast<encui_page *>(nullptr), _cancel_field};
static ui::button  _back{*static_cast<encui_page *>(nullptr), _back_field};
static ui::button  _next{*static_cast<encui_page *>(nullptr), _next_field};
...
encui_direct_key(uint16_t scancode)
{
    ...
    if ((VK_F8 == scancode) && !_checkbox_down)
    {
        ...
        if (checkbox_field)
        {
            (*(panel_->begin() + (checkbox_field - _page->fields)))->click(-1, -1);
        }
